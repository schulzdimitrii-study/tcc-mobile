...
O aplicativo mobile foi desenvolvido com Kotlin Multiplatform e Compose Multiplatform, adotando uma base compartilhada para interface, modelos de domínio, lógica de jogo e processamento de telemetria. As integrações dependentes de plataforma permanecem isoladas em implementações específicas, principalmente no Android, que concentra a coleta real de sensores, permissões, execução em primeiro plano e comunicação com recursos nativos. O iOS é mantido como um invólucro multiplataforma funcional, mas ainda sem paridade completa com o fluxo Android de telemetria e renderização KorGE.

A aplicação funciona como o ponto de contato do usuário com a sessão gamificada. Ela realiza autenticação, carrega as Hordas disponibilizadas pelo backend, permite configurar a distância da corrida, coleta sinais de movimento e biofeedback, atualiza a simulação local do jogo, acompanha o estado remoto da sessão e envia dados ao backend em tempo real.
...
No aplicativo, o \texttt{GameController} concentra apenas o estado necessário para a interface e para a renderização local da cena, expondo um \texttt{GameSnapshot} com distância, velocidade, pressão, risco, pontuação, tempo decorrido e resultado exibidos ao usuário. Esse snapshot pode ser calculado localmente pelo \texttt{RunCalculator} em cenários de simulação, pré-visualização ou indisponibilidade da conexão. Entretanto, em uma sessão online, os cálculos autoritativos da corrida ficam no backend: a posição da Horda, o estado oficial do jogo, o ranking e o resultado da sessão são recebidos pelo aplicativo por WebSocket e aplicados na tela de jogo. Assim, o mobile atua principalmente como coletor de telemetria e cliente de visualização, enquanto o servidor permanece como fonte de verdade da sessão.
