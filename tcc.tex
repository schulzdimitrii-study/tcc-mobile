...
O aplicativo mobile foi desenvolvido com Kotlin Multiplatform e Compose Multiplatform, adotando uma base compartilhada para interface, modelos de domínio e lógica de jogo, enquanto as integrações dependentes de plataforma ficam isoladas em implementações específicas do Android e do iOS. A aplicação funciona como o ponto de contato do usuário com a sessão gamificada: ela realiza autenticação, permite selecionar ou configurar a Horda, coleta sinais de movimento do dispositivo, atualiza a simulação local do jogo e envia dados ao backend em tempo real.
...
O pipeline de telemetria é uma das partes centrais do aplicativo. No Android, a entrada dos sinais ocorre por meio do \texttt{FusedLocationTrackingService}, que obtém localização via Play Services Location, e do \texttt{AndroidMotionSensorService}, que lê amostras de aceleração do dispositivo. A arquitetura também define o contrato \texttt{WearTelemetryBridge} para integração com smartwatch, embora a implementação atual utilize \texttt{NoOpWearTelemetryBridge}, indicando que a ponte com wearable ainda não foi implementada de fato.
